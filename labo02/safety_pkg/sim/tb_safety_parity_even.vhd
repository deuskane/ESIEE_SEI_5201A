...
entity tb_safety_parity_even is
end entity tb_safety_parity_even;

architecture sim of tb_safety_parity_even is

  constant C_SCOPE  : string := "TB_SAFETY_PARITY_EVEN";
...
begin
  ...
  enc_inst : safety_enc
    generic map (SAFETY_ALGO => USE_PARITY_EVEN)
    port map (
      data_i   => data_i,
      data_o   => enc_o
    );
  ...
  begin
    ...
    for i in 1 to 100 loop
      v_res := i + 1;
      assert encoded_size(i, SAFETY_PARITY_EVEN) = v_res report "Size missmatch for SAFETY_PARITY_EVEN and len=" & integer'image(i) severity failure;
      assert decoded_size(v_res, SAFETY_PARITY_EVEN) = i report "Decoded size missmatch for SAFETY_PARITY_EVEN and len=" & integer'image(i) severity failure;
    end loop;
